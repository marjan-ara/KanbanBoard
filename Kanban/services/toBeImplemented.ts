...
export const getActiveSprints = (): ISprint[] => {
  // https://aradespsm.api.crm4.dynamics.com/api/data/v9.2/arades_sprints
    const result = Xrm.WebApi.retrieveRecords("arades_sprint", "?$filter=statecode eq 0");
...
export const getSprintTasks = (date:Date) => {
    ...
    const result = Xrm.WebApi.retrieveRecords("arades_sprinttask", "?$filter=statecode eq 0 and Microsoft.Dynamics.CRM.On(PropertyName=@p1,PropertyValue=@p2)&@p1='arades_plannedstartdate'&@p2='${formattedDate}'");
    return result.entities;
}

export const getProjectTasks=()=>{
    //get all active and not-finished project tasks
    const result = Xrm.WebApi.retrieveRecords("arades_projecttask", "?$filter=statecode eq 0");
    return result.entities;
}
...
export const getOwners=()=>{
    ...
    const result = Xrm.WebApi.retrieveRecords("systemuser", "?$filter=isdisabled eq false and islicensed eq true");
    return result.entities;
}
...
export const updateProjectTask=(projectTaskId:number,ownerId:string, estimatedDuration:number,closeTask:boolean)=>{
    ...
    const task = Xrm.WebApi.retrieveRecord("arades_projecttask", projectTaskId, "?$select=*");
    return task;
}
...
export const updateSprintTask=(sprintTaskId:number,ownerId:string, estimatedDuration:number,closeTask:boolean)=>{
    ...
    const task = Xrm.WebApi.retrieveRecord("arades_sprinttask", sprintTaskId, "?$select=*");
    if (closeTask) {
        await Xrm.WebApi.updateRecord("arades_projecttask", task._arades_taskid_value, {
            statecode: 1,
